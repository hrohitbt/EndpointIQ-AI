...
export default function AgentDevices() {
  ...
  const healthScore = useMemo(() => {
    ...
    const bit = dashboard.security?.bitlocker || {};
    const def = dashboard.security?.defender || {};
    const fw = dashboard.security?.firewall || {};
    const patches = dashboard.security?.patches || {};
    const compliant = (bit.Compliant || 0) + (def.Compliant || 0) + (fw.Compliant || 0) + (patches.Compliant || 0);
    const total = dashboard.deviceCount * 4;
...
function Overview({ devices, stats, loading, healthScore, dashboard, trends }) {
  ...
  const patches = dashboard?.security?.patches || {};
  const compliant = (bit.Compliant || 0) + (def.Compliant || 0) + (fw.Compliant || 0) + (patches.Compliant || 0);
  const review = (bit.Review || 0) + (def.Review || 0) + (fw.Review || 0) + (patches.Review || 0);
  const unavailable = (bit["Data unavailable"] || 0) + (def["Data unavailable"] || 0) + (fw["Data unavailable"] || 0) + (patches["Data unavailable"] || 0);
  const controlsTotal = compliant + review + unavailable;
    ...
    <div className="mt-6 space-y-6">
      <section className="grid gap-6 rounded-2xl border border-slate-200 bg-[#0b1324] p-6 text-white shadow-sm lg:grid-cols-[auto_1fr_auto]">
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="mt-1 text-xs font-medium uppercase tracking-wide text-slate-400">Security posture score</p>
        </div>

        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Compliance trend</p>
            ...
            {trends.length > 1 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trends} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
                  ...
                  />
                  <Line type="monotone" dataKey="complianceRate" stroke="#60a5fa" strokeWidth={2.5} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="flex flex-col justify-center gap-2 text-sm">
          <div className="flex items-center justify-between gap-6 rounded-xl bg-white/5 px-3 py-2">
            ...
            <span className="font-semibold text-emerald-400">{compliant}/{controlsTotal || 0}</span>
          </div>
          <div className="flex items-center justify-between gap-6 rounded-xl bg-white/5 px-3 py-2">
            <span className="text-slate-300">Needs review</span>
            <span className="font-semibold text-amber-400">{review}/{controlsTotal || 0}</span>
          </div>
          <div className="flex items-center justify-between gap-6 rounded-xl bg-white/5 px-3 py-2">
            <span className="text-slate-300">Telemetry missing</span>
            <span className="font-semibold text-slate-400">{unavailable}/{controlsTotal || 0}</span>
          </div>
        </div>
      </section>
